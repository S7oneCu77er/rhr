...
if(!isset($_GET['confirm']) || $_GET['confirm'] != 'true') {
    ...
    <div class="page">
        ...
        text-shadow: 0.5px 0 currentColor, -0.5px 0 currentColor, 0 0.5px currentColor, 0 -0.5px currentColor;">
            Are you sure you want to delete site: ' . getSiteName($site_guid) . '?
        </div>
        <br>
        <h2 style="margin-top: 100px;">' . getSiteAddress($site_guid) . '</h2>
    </div>
    <div class="hours_page" style="margin-bottom: 20px;">
        ...
        <div id="shift_end">
            <button onclick="location.href=\'index.php?lang='.$selectedLanguage.'&page=admin&sub_page=sites\'">' . htmlspecialchars($lang_data[$selectedLanguage]['no_go_back'] ?? 'NO - GO BACK') . '</button>
        </div>
    </div>';
}
else if(isset($_GET['confirm']) && $_GET['confirm'] == 'true')
{
    ...
    if (deleteSite($site_guid)) {
        ...
        echo "<script>showError('" . htmlspecialchars($lang_data[$selectedLanguage]['worker_delete_error'] ?? 'Error deleting worker.') . "');</script>";
        header("Location: {$url}");
    }
} else {
    header("Location: index.php");
